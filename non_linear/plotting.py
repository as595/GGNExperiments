...
def plot_dataset(ax, problem, X, y):
    ...
    theta_0 = problem.theta_0.detach()
    problem.model.eval()
    vector_to_parameters(theta_0, problem.model.parameters())
    ys = problem.model(torch.from_numpy(xs.reshape(-1,1)).float()).detach().numpy()

    ax.plot(xs, ys, '--', label=r"$y = \theta_2 RELU (\theta_1 x)$", linewidth=1, color="k")
...
def plot_ldata(ax, problem, X, y, type='full'):
    ...
    problem.model.eval()
    vector_to_parameters(theta_0, problem.model.parameters())
    f_0 = problem.model(torch.from_numpy(xs.reshape(-1,1)).float()).detach().squeeze()
    J_theta = problem.J_theta_f(theta_0, x=torch.from_numpy(xs.reshape(-1,1)).float())
        
    ax.plot(xs, f_0.numpy(), '--', label=r"$y = \theta_2 RELU (\theta_1 x)$", linewidth=1, color="k")
    for i in range(len(samples)):
        
        theta = samples[i]
        d_theta = theta - theta_0
        ys = f_0 + J_theta @ d_theta
        
        ax.plot(xs, ys.detach().numpy(), '--', linewidth=1, alpha = 0.2, color="gray")
...
def plot_ypred(ax, problem, xs, type='sampled'):
    ...
    if type=='sampled':
        for i in range(len(samples)):
            theta = samples[i]
            vector_to_parameters(theta, problem.model.parameters())
            ys.append(problem.model(torch.from_numpy(xs.reshape(-1,1)).float()).detach().numpy())

        sns.kdeplot(np.array(ys), bw=0.5)
        pl.show()
...
def plot_loss_contour(ax, problem):
    ...
    ax.contour(thetas[0], thetas[1], losses.T, levels=[3,5,8,16], colors=["k"], alpha=0.3)

def plot_degeneracy(axes, problem):

    alphas = np.logspace(-1,2,30)
    
    theta = problem.theta_0
    thetas = theta.repeat(len(alphas), 1)
    for i in range(len(alphas)):
        thetas[i,0] *= alphas[i]
        thetas[i,1] /= alphas[i]

    for ax in axes:
        ax.plot(thetas[:,0], thetas[:,1], '--', linewidth=1, color="black")
...
def plot_samples(ax, problem):
    ...
    fdist = samples[ix_kernel,:]
    problem.fdist = fdist
    
    for sample_set in [samples]:
        ax.scatter(sample_set[:,0].detach().numpy(), sample_set[:,1].detach().numpy(), s=3, c='b')
...
def plot(X, y, problem, vec_funcs, starts, chains):
    ...
    plot_degeneracy(axes[1:], problem)
    ...
    for ax in axes[1:]:
        ...
        ax.set_xlabel(r"$\theta_1$", labelpad=0.5*LABELPAD_DIFF)
        ax.set_ylabel(r"$\theta_2$", labelpad=LABELPAD_DIFF)
...
def funcplot(X, y, problem, vec_funcs):
    ...
    plot_ldata(axes[1], problem, X, y, type='full')

    # plot the kernel contribution:
    plot_ldata(axes[2], problem, X, y, type='kernel')

    # plot the kernel contribution:
    plot_ldata(axes[3], problem, X, y, type='nonkernel')

    for i in range(1,4):
        axes[i].set_xlabel("x", labelpad=0.5*LABELPAD_DIFF)
        axes[i].set_ylabel("y", labelpad=0.75*LABELPAD_DIFF)
    ...
    axes[0].set_xlabel(r"$\theta_1$", labelpad=0.5*LABELPAD_DIFF)
    axes[0].set_ylabel(r"$\theta_2$", labelpad=LABELPAD_DIFF)
